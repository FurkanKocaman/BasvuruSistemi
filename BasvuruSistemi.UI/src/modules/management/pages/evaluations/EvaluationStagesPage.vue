...
const deleteStage = async (id: string) => {
  ...
  if (result) {
    const res = await evaluationService.deleteEvaluatonStage(id);
    if (res) {
      evaluationStages.value = evaluationStages.value.filter((p) => p.id != id);
    }
  }
};
</script>
<template>
  <main class="w-full h-full px-10 pt-20 pb-10">
    <ConfirmModal
      ref="confirmModal"
      title="Değerlendirme adımını silmek istediğinize emin misiniz?"
                ...
                >
                  ...
                  <td class="py-3 px-2">
                    ...
                    <button
                      ...
                      @click.stop="deleteStage(evaluationStage.id)"
                    >
                      <svg
                        class="size-5 group"
                        viewBox="0 0 24 24"
