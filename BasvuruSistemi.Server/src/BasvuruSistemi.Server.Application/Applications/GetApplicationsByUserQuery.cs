...
public sealed record GetApplicationsByUserQuery(
    int page,
    int pageSize
    ) : IRequest<Result<PagedResult<GetApplicationsByUserQueryResponse>>>;
...
internal sealed class GetApplicationsByUserQueryHandler(
    ...
    ) : IRequestHandler<GetApplicationsByUserQuery, Result<PagedResult<GetApplicationsByUserQueryResponse>>>
{
    public async Task<Result<PagedResult<GetApplicationsByUserQueryResponse>>> Handle(GetApplicationsByUserQuery request, CancellationToken cancellationToken)
    {
        Guid? userId = currentUserService.UserId;
        if (!userId.HasValue)
            return Result<PagedResult<GetApplicationsByUserQueryResponse>>.Failure(401,"User not found.");

        var applications = await applicationRepository
            .Where(p => userId.Value == p.UserId && !p.IsDeleted)
            .Include(p => p.JobPosting).ThenInclude(p => p.Unit)
        ...
        return Result<PagedResult<GetApplicationsByUserQueryResponse>>.Succeed(new PagedResult<GetApplicationsByUserQueryResponse>(response,request.page, request.pageSize,totalCount));
    }
}
